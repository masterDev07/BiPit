...
echo -e "${kuning}Tunggu sebentar ...${zero}"
	sleep 3
...
echo -e "${kuning}$nama_file${zero} Berhasil dibuat Binarinya menjadi \033[1;35m$namafilenya\033[0m"
	rm -rf $lokasi_direktoriasal/buatAwalshalat.spec
echo -e "${hijau}$lokasi_direktoriasal/buatAwalshalat.spec Berhasil dihapus${zero}"
echo -e "${kuning}Silakan tunggu Sebentar cuma 2 detik aja${zero}"
	sleep 2
}
...
echo -e "${kuning}Hapus file dot spec$zero $lokasi_direktoriasal/$namafilenya.spec"
	rm -rf $lokasi_direktoriasal/$namafilenya.spec
echo -e "${hijau}Hapus direktori dist spec$zero $lokasi_direktoriasal/dist"
	rm -rf $lokasi_direktoriasal/dist
echo -e "${kuning}Hapus diirektori build$zero $lokasi_direktoriasal/build"
	rm -rf $lokasi_direktoriasal/build
if [ -f $lokasi_direktoriasal/__pycache__ ];then
echo -e "${hijau}Hapus direktori build$zero $lokasi_direktoriasal/__pycache__"
...
if [[ $pilihPutaratauTidakmp3 == "s" ]];then
echo -e "file Audio tidak Diputar"
else
buat_memutaraudiodaftarjamataumenit $filemp3
fi
	
echo -e "${hijau}Lokasi direktori Terakhir adalah di $lokasi_direktoriasal${zero}"
echo -e "${kuning}Alkhamdu lillah. Ya Allah semoga program $namafilenya bermanfaat dunia akhirat.${zero} ${biru}Semoga doaku Dikabulkan Ya Allah.${zero}"    
}
